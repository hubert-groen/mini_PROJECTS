...
class HeapV_h:
    ...
    def heapify_down(self, index):
        ...
        for i in range(1, self.degree + 1):
            ...
            if child_index <= self.last_index and self.heap_list[child_index] > self.heap_list[max_index]:
                max_index = child_index
        if max_index != index:
            self.heap_list[max_index], self.heap_list[index] = self.heap_list[index], self.heap_list[max_index]
            self.heapify_down(max_index)


def draw_heap_h(heap):
    G = nx.Graph()
    node_labels = {}
    node_colors = []
